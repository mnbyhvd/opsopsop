...
interface SortableItemProps {
  item: TechnicalSpec;
  index: number;
  onEdit: (item: TechnicalSpec) => void;
...
const TechnicalSpecsAdmin: React.FC = () => {
  const [items, setItems] = useState<TechnicalSpec[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingItem, setEditingItem] = useState<TechnicalSpec | null>(null);
  ...
  const fetchItems = async () => {
    try {
      const response = await fetch('/api/technical-specs');
      const data = await response.json();
      if (data.success) {
        setItems(data.data.sort((a: TechnicalSpec, b: TechnicalSpec) => a.sort_order - b.sort_order));
      }
    } catch (error) {
  ...
  const handleEdit = (item: TechnicalSpec) => {
    setEditingItem({ ...item });
    setIsCreating(false);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const url = isCreating
        ? '/api/technical-specs'
        : `/api/technical-specs/${editingItem.id}`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/technical-specs/${id}`, {
        method: 'DELETE',
      });
    ...
    if (over && active.id !== over.id) {
      ...
      try {
        ...
        const updatePromises = updates.map(update => 
          fetch(`/api/technical-specs/${update.id}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
            },
      ...
      {editingItem && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
          <div className="admin-card max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            >
              {isCreating ? 'Создание новой характеристики' : 'Редактирование характеристики'}
            </h2>
...
export default TechnicalSpecsAdmin;